...
__title__ = (
    "fobi.contrib.apps.drf_integration.form_elements.fields." "input.base"
)
...
class InputPlugin(IntegrationFormFieldPlugin, DRFSubmitPluginFormDataMixin):
    """CharField plugin."""
    ...
    def get_custom_field_instances(
        ...
        **kwargs,
        ...
        if form_element_plugin.data.step_value:
            field_metadata.update({"step": form_element_plugin.data.step_value})

        if (
            form_element_plugin.data.type_value
            and form_element_plugin.data.type_value
            in (
                "submit",
                "button",
                "reset",
