...
__title__ = (
    ...
    "select_multiple_model_objects.conf"
)
...
def get_setting(setting, override=None):
    ...
    if hasattr(
        settings,
        "FOBI_FORM_ELEMENT_SELECT_MULTIPLE_MODEL_OBJECTS_{0}".format(setting),
    ):
        return getattr(
            settings,
            "FOBI_FORM_ELEMENT_SELECT_MULTIPLE_MODEL_OBJECTS_{0}".format(
                setting
            ),
        )
    else:
