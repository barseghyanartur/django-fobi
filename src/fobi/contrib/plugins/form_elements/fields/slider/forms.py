...
class SliderInputForm(forms.Form, BaseFormFieldPluginForm):
    ...
    def clean(self):
        ...
        if step > max_value - min_value:
            self.add_error(
                "step", _("`step` should be > than `max_value` - `min_value`.")
            )
        ...
        if handle in label_handles and show_endpoints_as in tick_endpoints:
            self.add_error(
                "handle",
                _(
                    "You are not allowed to use Triangle or Custom handles "
                    "with ticks enabled."
                ),
            )
        ...
        if custom_ticks:
            ticks = get_select_field_choices(
                custom_ticks, key_type=int, value_type=str, fail_silently=False
            )
            if ticks is None:
                self.add_error(
                    "custom_ticks",
                    _(
                        "Invalid format. First value should be an integer, "
