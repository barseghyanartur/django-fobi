...
from fobi.contrib.plugins.form_handlers.mail import defaults
...
def get_setting(setting, override=None):
    ...
    if override is not None:
        return override
    if hasattr(settings, 'FOBI_PLUGIN_MAIL_{0}'.format(setting)):
        return getattr(settings, 'FOBI_PLUGIN_MAIL_{0}'.format(setting))
    else:
